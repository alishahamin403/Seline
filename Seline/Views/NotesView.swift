...
struct NotesView: View, Searchable {
    ...
    private enum NotesContentFilter: String, CaseIterable {
        case notes = "Notes"
    ...
    @State private var showSidebarNotesSelection = false
    ...
    @Namespace private var contentFilterAnimation
    @State private var selectedContentFilter: NotesContentFilter = .notes
    ...
    @State private var showJournalHub = false
    @State private var showAllNotesArchive = false
    @State private var journalOpenTodayOnHubAppear = false
    @State private var journalScrollToHistoryOnHubAppear = false
    @State private var journalDraftToOpen: JournalDraft? = nil
    @State private var journalExistingNoteToOpen: Note? = nil
    ...
    @StateObject private var hubState = NotesHubState()

    private var cachedFilteredPinned: [Note] { hubState.filteredPinnedNotes }
    private var cachedRecentNotes: [Note] { hubState.recentNotes }
    private var cachedNotesByMonth: [(month: String, notes: [Note])] { hubState.notesByMonth }
    private var isPinnedNotesSelection: Bool {
        showSidebarNotesSelection && selectedFolderId == nil && !showUnfiledNotesOnly
    }
    private var isNotesHomeSelection: Bool {
        selectedFolderId == nil && !showUnfiledNotesOnly && !isPinnedNotesSelection
    }
    ...
    private func refreshNoteCaches() {
        hubState.updateInputs(
            ...
            showPinnedNotesOnly: isPinnedNotesSelection,
    ...
    private var hubDisplayedNotes: [Note] {
        hubState.displayedNotes
    }

    private var isShowingNotesBrowseSelection: Bool {
        selectedMainPage == .notes && (showSidebarNotesSelection || selectedFolderId != nil || showUnfiledNotesOnly)
    }
    ...
    private var notesHeroPinnedRailTitle: String {
        showUnfiledNotesOnly ? "Pinned" : "Pinned notes"
    }
    ...
    private var notesHomeRecentHighlightNotes: [Note] {
        let pinnedIds = Set(cachedFilteredPinned.map(\.id))
        let recentCandidates = cachedRecentNotes.filter { !pinnedIds.contains($0.id) }

        if !recentCandidates.isEmpty {
            return Array(recentCandidates.prefix(3))
        }

        let fallback = hubDisplayedNotes.filter { !pinnedIds.contains($0.id) }
    ...
    private var notesThisWeekNotes: [Note] {
    ...
    private var notesHeroContextMetric: (title: String, value: String) {
        if showUnfiledNotesOnly {
            return ("Visible", "\(hubState.looseNotesCount)")
        }

        if selectedFolderId != nil {
            return ("In folder", "\(hubState.folderNotesCount)")
        }

        return ("Unfiled", "\(hubState.looseNotesCount)")
    }

    private var notesHeroPinnedNotes: [Note] {
        Array(cachedFilteredPinned.prefix(8))
    }

    private var latestJournalRecap: Note? {
        hubState.latestJournalRecap
    }

    private var journalOverviewStats: JournalStats {
        hubState.journalOverviewStats
    }

    private var journalOverviewWeekProgress: String {
        "\(journalOverviewStats.completedThisWeek) / 7"
    }

    private var floatingActionPage: NotesFloatingActionPage {
    ...
    private var shouldShowJournalOverview: Bool {
        selectedFolderId == nil && !showUnfiledNotesOnly
    }
    ...
    private var isShellFloatingActionVisible: Bool {
        ...
        !showAllNotesArchive &&
        noteForReminder == nil &&
        journalDraftToOpen == nil &&
        journalExistingNoteToOpen == nil
    }
    ...
    private func syncFloatingActionState() {
        floatingActionCoordinator.updateNotes(
            isVisible: isShellFloatingActionVisible,
            page: floatingActionPage
        )
    }
    ...
    var body: some View {
        NavigationStack(path: $navigationPath) {
                            ...
                            VStack(spacing: 0) {
                                if !isSearchActive {
                                    HStack(spacing: 10) {
                                        ...
                                        if selectedMainPage == .notes && !isShowingNotesBrowseSelection {
                                            notesContentFilterRow
                                        } else {
                                            Spacer(minLength: 0)
                                            Text(headerTitle)
            ...
            .navigationDestination(isPresented: $showJournalHub) {
                JournalHubView(
                    openTodayOnAppear: journalOpenTodayOnHubAppear,
                    onConsumeOpenToday: { journalOpenTodayOnHubAppear = false },
                    scrollToHistoryOnAppear: journalScrollToHistoryOnHubAppear,
                    onConsumeScrollToHistory: { journalScrollToHistoryOnHubAppear = false },
                    isEmbeddedInNotesShell: false
                )
            }
            .navigationDestination(isPresented: $showAllNotesArchive) {
                NotesArchiveView(
                    onOpenNote: { note in
        ...
        .onChange(of: selectedContentFilter) { _ in refreshNoteCaches() }
        .onChange(of: selectedMainPage) { _ in syncFloatingActionState() }
        .onChange(of: isSearchActive) { _ in syncFloatingActionState() }
        .onChange(of: showingFolderSidebar) { _ in syncFloatingActionState() }
        .onChange(of: isSidebarOverlayVisible) { _ in syncFloatingActionState() }
        ...
        .onReceive(NotificationCenter.default.publisher(for: .openJournalFromMainApp)) { notification in
            ...
            let openToday = (notification.userInfo?["openToday"] as? Bool) ?? false
            if openToday {
                openTodayJournalEntry()
            }
        }
        .onReceive(NotificationCenter.default.publisher(for: .openReceiptsFromMainApp)) { _ in
            selectedMainPage = .notes
            selectedFolderId = nil
            showUnfiledNotesOnly = false
            showSidebarNotesSelection = false
            ...
            isSearchFocused = false
            searchText = ""
            showRecurringOverview = false
            showReceiptStats = true
        }
        .onReceive(NotificationCenter.default.publisher(for: .openRecurringFromMainApp)) { _ in
            selectedMainPage = .notes
            selectedFolderId = nil
            showUnfiledNotesOnly = false
            showSidebarNotesSelection = false
        ...
        .onReceive(NotificationCenter.default.publisher(for: .notesShellAddRequested)) { _ in
            performMainPageAddAction()
        }
        .onReceive(NotificationCenter.default.publisher(for: .notesShellNewNoteRequested)) { _ in
            openNewNoteEditor()
        }
        .onReceive(NotificationCenter.default.publisher(for: .notesShellNewJournalRequested)) { _ in
            openTodayJournalEntry()
        }
        ...
        .fullScreenCover(item: $journalExistingNoteToOpen, onDismiss: {
            notesManager.isViewingNoteInNavigation = false
        }) { note in
            NoteEditView(
                note: note,
                isPresented: Binding(
                    get: { journalExistingNoteToOpen != nil },
                    set: { if !$0 { journalExistingNoteToOpen = nil } }
                )
            )
        }
        .fullScreenCover(item: $journalDraftToOpen, onDismiss: {
            notesManager.isViewingNoteInNavigation = false
        }) { draft in
            NoteEditView(
                note: nil,
                isPresented: Binding(
                    get: { journalDraftToOpen != nil },
                    set: { if !$0 { journalDraftToOpen = nil } }
                ),
    ...
    private func processReceiptImageDirectly(_ image: UIImage) {
        Task {
            ...
            do {
                let draft = try await openAIService.analyzeReceiptImageDraft(image)
    ...
    private func processNextReceiptInQueue() {
        ...
        Task {
            await MainActor.run {
                receiptProcessingState = totalCount > 1
                    ? .processingMultiple(current: currentNumber, total: totalCount)
                    : .processing
            }
    ...
    private var mainTabContent: some View {
        switch selectedMainPage {
        case .notes:
            if selectedContentFilter == .journal {
                JournalHubView(
                    openTodayOnAppear: false,
                    onConsumeOpenToday: nil,
                    scrollToHistoryOnAppear: false,
                    onConsumeScrollToHistory: nil,
                    isEmbeddedInNotesShell: true
                )
            } else {
    ...
    private var notesPageTabSelector: some View {
        HStack(spacing: 6) {
            ForEach(NotesMainPage.allCases, id: \.self) { page in
                ...
                Button(action: {
                    HapticManager.shared.selection()
                    withAnimation(.spring(response: 0.3, dampingFraction: 0.75)) {
                        if page == .notes && selectedMainPage == .notes {
                            showSidebarNotesSelection = false
                            selectedFolderId = nil
                            showUnfiledNotesOnly = false
                        } else {
        ...
        .overlay(
            Capsule()
                .strokeBorder(tabContainerStrokeColor(), lineWidth: 1)
        )
    }

    // MARK: - Notes/Journal Filter Row

    private var notesContentFilterRow: some View {
        HStack(spacing: 6) {
            ForEach(NotesContentFilter.allCases, id: \.self) { filter in
                    ...
                    Text(filter.rawValue)
                        ...
                        .background {
                            if isSelected {
                                Capsule()
                                    .fill(tabBackgroundColor())
                                    .matchedGeometryEffect(id: "notesContentFilter", in: contentFilterAnimation)
                            }
                        }
                }
                .buttonStyle(PlainButtonStyle())
    ...
    // MARK: - Unified Stats Banner

    private var unifiedStatsBanner: some View {
        HStack(spacing: 10) {
            switch selectedContentFilter {
            case .notes:
                notesHeroMetricTile(title: "Pinned", value: "\(cachedFilteredPinned.count)")
                notesHeroMetricTile(title: "Recent 7d", value: "\(cachedRecentNotes.count)")
                notesHeroMetricTile(title: notesHeroContextMetric.title, value: notesHeroContextMetric.value)
            case .journal:
                notesHeroMetricTile(title: "Streak", value: "\(journalOverviewStats.currentStreak)d")
                notesHeroMetricTile(title: "This week", value: journalOverviewWeekProgress)
                notesHeroMetricTile(title: "Entries", value: "\(journalOverviewStats.totalEntries)")
            }
        }
    }

    // MARK: - Journal Recap Section

    @ViewBuilder
    private var journalRecapSection: some View {
        journalOverviewMiniCard(
            title: "Latest recap",
            body: latestJournalRecap?.preview ?? "Weekly recaps appear here once you start journaling.",
            makesWholeCardTappable: latestJournalRecap != nil,
            action: {
                guard let latestJournalRecap else { return }
                journalExistingNoteToOpen = latestJournalRecap
            }
        )
    }
    ...
    private func journalEntryRow(for note: Note) -> some View {
        HStack(spacing: 12) {
            ZStack {
                RoundedRectangle(cornerRadius: 10)
                    .fill(Color.homeGlassAccent.opacity(colorScheme == .dark ? 0.18 : 0.13))
            ...
            VStack(alignment: .leading, spacing: 4) {
                let entryDate = note.journalDate ?? note.dateModified
                Text(journalEntryDateTitle(for: entryDate))
            ...
            if let mood = note.journalMood {
                Text(mood.title)
                    ...
                    .foregroundColor(Color.homeGlassAccent.opacity(0.85))
                    .lineLimit(1)
            }
        }
    ...
    private func journalEntryDateTitle(for date: Date) -> String {
        let calendar = Calendar.current
    ...
    private var unifiedContentList: some View {
        VStack(spacing: 14) {
            ...
            case .notes:
                unifiedNotesContent
            case .journal:
                unifiedJournalContent
            }
        }
    }

    @ViewBuilder
    private var unifiedNotesContent: some View {
        if isNotesHomeSelection {
            if !cachedFilteredPinned.isEmpty {
                notesListSectionCard(
                    title: "Pinned",
                    notes: cachedFilteredPinned
                )
            }
            ...
            ForEach(cachedNotesByMonth, id: \.month) { group in
                notesListSectionCard(
                    title: group.month,
                    notes: group.notes
                )
        ...
        if hubDisplayedNotes.isEmpty {
            unifiedEmptyState(filter: .notes)
        }
    }

    @ViewBuilder
    private var unifiedJournalContent: some View {
        if hubState.journalEntries.isEmpty {
            unifiedEmptyState(filter: .journal)
        } else {
            let grouped = groupJournalEntriesByWeek(hubState.journalEntries)
    ...
    private func groupJournalEntriesByWeek(_ entries: [Note]) -> [(label: String, entries: [Note])] {
        let calendar = Calendar.current
        let now = Date()
        var groups: [(label: String, entries: [Note])] = []
        var seen = Set<String>()

        for entry in entries {
            let date = entry.journalDate ?? entry.dateModified
            let weekStart = calendar.dateInterval(of: .weekOfYear, for: date)?.start ?? date
            let weekKey = calendar.startOfDay(for: weekStart).description

            if !seen.contains(weekKey) {
                seen.insert(weekKey)
                let label: String
                if let thisWeek = calendar.dateInterval(of: .weekOfYear, for: now),
                   thisWeek.contains(date) {
                    ...
                    label = "Week of \(formatter.string(from: weekStart))–\(formatter.string(from: end))"
                }
    ...
    private func unifiedEmptyState(filter: NotesContentFilter) -> some View {
        VStack(spacing: 12) {
            Image(systemName: filter == .journal ? "book.closed" : "note.text")
                .font(.system(size: 40, weight: .light))
                .foregroundColor(Color.appTextSecondary(colorScheme).opacity(0.5))

            Text(filter == .journal ? "No journal entries yet" : "No notes yet")
    ...
    private var hubNotesSection: some View {
        VStack(spacing: 0) {
            ...
            if hubDisplayedNotes.isEmpty {
                hubEmptyState(
                    icon: "note.text",
                    title: showUnfiledNotesOnly
                        ? "No unfiled notes"
                        : (selectedFolderId != nil ? "No notes in folder" : "No pinned notes"),
                    subtitle: showUnfiledNotesOnly
                        ? "All loose notes are cleared"
                        : (selectedFolderId != nil ? "This folder does not have any notes yet" : "Pin notes to keep them here")
                )
                .padding(.horizontal, 14)
                .padding(.bottom, 14)
            } else {
                notesListRows(hubDisplayedNotes)
            }
        }
        .appAmbientCardStyle(
            colorScheme: colorScheme,
    ...
    private var notesDashboardHeroCard: some View {
        VStack(alignment: .leading, spacing: 16) {
            HStack(alignment: .top, spacing: 12) {
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text("Ideas, references, and saved details")
                ...
                Button(action: {
                    ...
                    Image(systemName: "square.and.pencil")
                        ...
                        .background(
                            Circle()
                                .fill(notesDashboardAccentColor)
                        )
                }
                .buttonStyle(PlainButtonStyle())
    ...
    private var notesOverviewCard: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            if notesHeroPinnedNotes.isEmpty {
                notesOverviewEmptyPinnedState
            } else {
                embeddedPinnedNotesOverview
            }
        }
        .padding(16)
        .appAmbientCardStyle(
    ...
    @ViewBuilder
    private var notesFloatingAddButton: some View {
    ...
    private var journalOverviewCard: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            journalOverviewMiniCard(
                title: "Latest recap",
                body: latestJournalRecap?.preview ?? "Weekly recaps appear here once you start journaling and keep updating as the week fills in.",
                makesWholeCardTappable: latestJournalRecap != nil,
                action: {
                    guard let latestJournalRecap else { return }
                    journalExistingNoteToOpen = latestJournalRecap
    ...
    private var embeddedPinnedNotesOverview: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text(notesHeroPinnedRailTitle)
                .font(FontManager.geist(size: 13, weight: .semibold))
                .foregroundColor(Color.appTextPrimary(colorScheme))
    ...
    private var notesOverviewEmptyPinnedState: some View {
        VStack(spacing: 8) {
            Image(systemName: "pin.slash")
                .font(.system(size: 18, weight: .regular))
                .foregroundColor(Color.appTextSecondary(colorScheme))

            Text(showUnfiledNotesOnly ? "No pinned loose notes" : "No pinned notes yet")
                .font(FontManager.geist(size: 14, weight: .semibold))
                .foregroundColor(Color.appTextPrimary(colorScheme))

            Text("Pin the notes you reach for most and they will stay anchored here.")
                .font(FontManager.geist(size: 12, weight: .regular))
                .foregroundColor(Color.appTextSecondary(colorScheme))
                .multilineTextAlignment(.center)
        }
    ...
    @ViewBuilder
    private func journalOverviewMiniCard(
        title: String,
        body: String,
        makesWholeCardTappable: Bool = false,
    ...
    private var searchResultsOverlay: some View {
        ZStack(alignment: .top) {
            Color.black.opacity(colorScheme == .dark ? 0.58 : 0.16)
                .onTapGesture {
                    isSearchFocused = false
                }
            ...
            if searchResults.isEmpty {
                ...
                ForEach(Array(searchResults), id: \.id) { note in
                    Button(action: {
                        ...
                        HStack(spacing: 12) {
                            // Icon
                            Image(systemName: note.isJournalWeeklyRecap ? "book.closed.fill" : (note.isJournalEntry ? "square.and.pencil" : (note.isPinned ? "pin.fill" : "doc.text")))
                                .font(FontManager.geist(size: 14, weight: .regular))
    ...
    private var receiptSearchResults: some View {
        ...
        let filteredReceipts = receiptManager.receipts
            .filter { receipt in
                query.isEmpty || receipt.searchableText.lowercased().contains(query)
            }
            .prefix(10)
    ...
    @ViewBuilder
    private func receiptSearchResultsContent(filteredReceipts: [ReceiptStat]) -> some View {
        VStack(spacing: 0) {
    ...
    @ViewBuilder
    private func receiptResultsList(filteredReceipts: [ReceiptStat]) -> some View {
        ForEach(filteredReceipts, id: \.id) { receipt in
            ...
            if filteredReceipts.last?.id != receipt.id {
                Divider()
                    .padding(.leading, 52)
            }
        }
    ...
    private func receiptRow(receipt: ReceiptStat) -> some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                Text("\(CurrencyParser.formatAmount(receipt.amount)) • \(FormatterCache.shortDate.string(from: receipt.date))")
                    .font(FontManager.geist(size: 12, weight: .regular))
                    .foregroundColor(Color.appTextSecondary(colorScheme))
                    .lineLimit(1)
            }
            ...
            Text(receipt.date.formatted(.relative(presentation: .named)))
                .font(FontManager.geist(size: 11, weight: .regular))
                .foregroundColor(Color.appTextSecondary(colorScheme))
        }
        .padding(.horizontal, 16)
    ...
    private func notesHeroMetricTile(title: String, value: String) -> some View {
        VStack(alignment: .leading, spacing: 6) {
            Text(title)
                .font(FontManager.geist(size: 11, weight: .medium))
                .foregroundColor(Color.appTextSecondary(colorScheme))
    ...
    private func notesListSectionCard(title: String, notes: [Note]) -> some View {
        VStack(spacing: 0) {
            HStack(spacing: 6) {
    ...
    @ViewBuilder
    private func notesListRows(_ notes: [Note]) -> some View {
        VStack(spacing: 0) {
    ...
    private func openJournalHub(openToday: Bool, scrollToHistory: Bool = false) {
        selectedMainPage = .notes
        selectedFolderId = nil
        showUnfiledNotesOnly = false
        showSidebarNotesSelection = false
        journalOpenTodayOnHubAppear = openToday
        journalScrollToHistoryOnHubAppear = scrollToHistory
        showJournalHub = true
    }

    private func openTodayJournalEntry() {
        if let existingEntry = notesManager.journalEntry(for: Date()) {
            journalExistingNoteToOpen = existingEntry
        } else {
            Task {
                journalDraftToOpen = await journalService.prepareTodayDraftEnsuringFolder()
            }
        }
    }
    ...
    private func activateNotesSearch() {
        HapticManager.shared.selection()
        withAnimation(.easeInOut(duration: 0.2)) {
            isSearchActive = true
    ...
    private func notesHeroPreview(for note: Note) -> String {
        let trimmed = note.preview.trimmingCharacters(in: .whitespacesAndNewlines)
        let singleLine = trimmed.replacingOccurrences(of: "\n", with: " ")
        if singleLine.count <= 86 {
            return singleLine
        }

        let endIndex = singleLine.index(singleLine.startIndex, offsetBy: 86)
        return String(singleLine[..<endIndex]) + "..."
    }

    private func notesHeroRelativeDate(for date: Date) -> String {
        let formatter = RelativeDateTimeFormatter()
        formatter.unitsStyle = .abbreviated
        return formatter.localizedString(for: date, relativeTo: Date())
    }

    private func notesHeroReminderText(for date: Date) -> String {
        let calendar = Calendar.current
        if calendar.isDateInToday(date) {
            return "Today"
        }
    ...
    private func notesTopActivityLabels(in notes: [Note]) -> [String] {
        Dictionary(grouping: notes) { notesFolderLabel(for: $0) }
    ...
    private func notesFolderLabel(for note: Note) -> String {
        guard let folderId = note.folderId else { return "Loose notes" }
        let folderName = notesManager.getFolderName(for: folderId).trimmingCharacters(in: .whitespacesAndNewlines)
        return folderName.isEmpty ? "Notes" : folderName
    }

    private var notesDashboardAccentColor: Color {
        Color(red: 0.98, green: 0.64, blue: 0.41)
    }

    private var notesDashboardHeroBackground: some View {
        RoundedRectangle(cornerRadius: 24)
            .fill(Color.appSurface(colorScheme))
            .overlay(alignment: .topTrailing) {
                Circle()
                    .fill(notesDashboardAccentColor.opacity(colorScheme == .dark ? 0.14 : 0.22))
    ...
    private var notesDashboardSectionCardBackground: some View {
...
struct NoteEditView: View {
    ...
    let initialNoteKind: NoteKind?
    let initialJournalDate: Date?
    let initialJournalWeekStartDate: Date?
    ...
    @State private var selectedFolderId: UUID? = nil
    @State private var noteKind: NoteKind? = nil
    @State private var journalDate: Date? = nil
    @State private var journalWeekStartDate: Date? = nil
    ...
    @State private var pendingTitleFocusWorkItems: [DispatchWorkItem] = []

    // File attachment states
    @State private var attachment: NoteAttachment?
    @State private var extractedData: ExtractedData?
    @State private var extractedDataSheet: ExtractedData?
    ...
    init(
        ...
        initialNoteKind: NoteKind? = nil,
        initialJournalDate: Date? = nil,
        initialJournalWeekStartDate: Date? = nil
    ) {
        self.note = note
        ...
        self.initialNoteKind = initialNoteKind
        self.initialJournalDate = initialJournalDate
        self.initialJournalWeekStartDate = initialJournalWeekStartDate
    }
    ...
    var body: some View {
        ...
        .sheet(item: $extractedDataSheet) { extractedData in
        ...
        .sheet(item: $filePreviewItem) { item in
            FilePreviewSheet(fileURL: item.url)
                .presentationBg()
        }
        .sheet(isPresented: $showingRecurringExpenseForm) {
            RecurringExpenseForm { expense in
    ...
    private var mainContentView: some View {
            ...
            VStack(spacing: 0) {
                ...
                ScrollView(.vertical, showsIndicators: false) {
                    VStack(spacing: 0) {
                        ...
                        Color.clear
                            .frame(height: isLockedInSession ? 24 : (isWeeklyRecapEditor ? 40 : 200))
                    }
        ...
        .safeAreaInset(edge: .bottom) {
            if !isLockedInSession && !isWeeklyRecapEditor {
                bottomActionButtons
                    .padding(.horizontal, 12)
                    .padding(.top, 8)
                    .padding(.bottom, 8)
    ...
    private var activeEditorKind: NoteKind {
        if let noteKind {
            return noteKind
        }
        if let editingNote {
            return editingNote.resolvedKind
        }
        if let note {
            return note.resolvedKind
        }
        return initialNoteKind ?? .standard
    }

    private var isJournalStyledNote: Bool {
        activeEditorKind == .journalEntry || activeEditorKind == .journalWeeklyRecap
    }

    private var isJournalEntryEditor: Bool {
        activeEditorKind == .journalEntry
    }

    private var isWeeklyRecapEditor: Bool {
        activeEditorKind == .journalWeeklyRecap
    }

    @ViewBuilder
    private var activeTopToolbar: some View {
        if isWeeklyRecapEditor {
            weeklyRecapToolbar
        } else {
            customToolbar
        }
    ...
    @ViewBuilder
    private var activeNoteBodyContent: some View {
        if isWeeklyRecapEditor {
            weeklyRecapReadOnlyContent
        } else {
            noteContentView
        }
    }
    ...
    private var journalCanvasMinHeight: CGFloat {
        max(UIScreen.main.bounds.height * 0.62, 420)
    }

    private var secondaryToolbarIconColor: Color {
        colorScheme == .dark ? .white : .black
    }

    private var secondaryToolbarCircleFill: Color {
        if isJournalStyledNote {
            return colorScheme == .dark
                ? journalAccentColor.opacity(0.18)
    ...
    private var primarySaveCircleFill: Color {
        isJournalStyledNote ? journalAccentColor : (colorScheme == .dark ? Color.white : Color.black)
    }

    private var primarySaveIconColor: Color {
        if isJournalStyledNote {
            return .black
        }
        return colorScheme == .dark ? .black : .white
    ...
    private var journalHeaderBadgeTitle: String {
        isJournalEntryEditor ? "Journal" : "Weekly recap"
    }

    private var journalHeaderSubtitle: String {
        if isJournalEntryEditor {
            return journalHeaderAISummary ?? journalHeaderFallbackSummary
        }
        return "A saved summary of the finished week, kept separate from regular notes."
    }

    private var journalHeaderFallbackSummary: String {
        ...
        guard !normalized.isEmpty else {
            return "No summary yet for this journal entry."
        }

        let preview = normalized.count > 130 ? String(normalized.prefix(130)).trimmingCharacters(in: .whitespacesAndNewlines) + "..." : normalized
        return preview
    }

    private var journalHeaderDateLabel: String {
        let formatter = DateFormatter()
        if isJournalEntryEditor {
            formatter.dateFormat = "EEEE, MMM d"
            return formatter.string(from: journalDate ?? Date())
        }

        formatter.dateFormat = "MMM d"
        let weekStart = journalWeekStartDate ?? Date()
    ...
    private var weeklyRecapMoodEntries: [WeeklyRecapMoodEntry] {
        ...
        return notesManager.journalEntries
            .compactMap { entry -> WeeklyRecapMoodEntry? in
                ...
                guard normalizedEntryDate >= normalizedWeekStart, normalizedEntryDate < weekEnd else {
                    return nil
                }
            ...
            if isJournalStyledNote {
                LinearGradient(
                    colors: colorScheme == .dark
                        ...
                        : [
                            journalAccentColor.opacity(0.16),
                            Color(red: 0.996, green: 0.977, blue: 0.94),
                            Color.white
                        ],
        ...
        HStack(spacing: 12) {
            ...
            Button(action: {
                ...
                Image(systemName: "chevron.left")
                    ...
                    .foregroundColor(secondaryToolbarIconColor)
                    .frame(width: 36, height: 36)
                    .background(Circle().fill(secondaryToolbarCircleFill))
            }
            ...
            .opacity((note != nil && !isWeeklyRecapEditor) ? 1.0 : 0.35)
            .disabled(note == nil || isWeeklyRecapEditor)
            ...
            Button(action: {
                ...
                Image(systemName: "checkmark")
                    ...
                    .foregroundColor(primarySaveIconColor)
                    .frame(width: 36, height: 36)
                    .background(Circle().fill(primarySaveCircleFill))
            }
        }
        .padding(.horizontal, 20)
        .padding(.top, isJournalStyledNote ? 8 : 2)
        .padding(.bottom, 20)
    }

    private var weeklyRecapToolbar: some View {
        HStack {
            Button(action: closeReadOnlyRecap) {
                Image(systemName: "chevron.left")
                    .font(FontManager.geist(size: 16, weight: .medium))
                    .foregroundColor(secondaryToolbarIconColor)
                    .frame(width: 36, height: 36)
    ...
    private var noteContentView: some View {
        ...
        return VStack(alignment: .leading, spacing: isJournalStyledNote ? 16 : 12) {
            if isJournalEntryEditor {
                journalEditorIdentityCard
                    .padding(.horizontal, 12)
                    .padding(.top, 4)
            }
            ...
            .padding(.horizontal, 12)
            .padding(.top, isJournalEntryEditor ? 2 : 4)
            .onChange(of: title) { _ in
                // Save to undo history on title change
                saveToUndoHistory()
            }
            
            // Content editor with table support
            ...
            .frame(
                maxWidth: .infinity,
                minHeight: isJournalStyledNote ? journalCanvasMinHeight : 100,
                alignment: .topLeading
            )

            if isWeeklyRecapEditor && !weeklyRecapMoodEntries.isEmpty {
                weeklyRecapMoodSummaryCard
                    .padding(.horizontal, 12)
            }
    ...
    private var weeklyRecapReadOnlyContent: some View {
        VStack(alignment: .leading, spacing: 24) {
            ...
            if !weeklyRecapMoodEntries.isEmpty {
                weeklyRecapMoodSummaryCard
                    .padding(.horizontal, 12)
            }
        }
    ...
    private var journalEditorIdentityCard: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 8) {
                Text(journalHeaderBadgeTitle)
                    .font(FontManager.geist(size: 12, weight: .semibold))
                    ...
                    .background(
                        Capsule()
                            .fill(journalAccentColor.opacity(colorScheme == .dark ? 0.22 : 0.18))
                    )

                Text(journalHeaderDateLabel)
                    .font(FontManager.geist(size: 12, weight: .medium))
                    .foregroundColor(colorScheme == .dark ? Color.white.opacity(0.64) : Color.black.opacity(0.56))
            }

            Text(journalHeaderSubtitle)
                .font(FontManager.geist(size: 14, weight: .regular))
            ...
            RoundedRectangle(cornerRadius: 26)
                .fill(
                    LinearGradient(
                        colors: colorScheme == .dark
                            ? [
                                journalAccentColor.opacity(0.2),
                                Color.white.opacity(0.05)
                            ]
                            : [
                                Color.white.opacity(0.82),
                                journalAccentColor.opacity(0.12)
                            ],
                        startPoint: .topLeading,
                        endPoint: .bottomTrailing
                    )
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 26)
                .stroke(
                    journalAccentColor.opacity(colorScheme == .dark ? 0.3 : 0.2),
                    lineWidth: 1
                )
        )
        .shadow(color: journalAccentColor.opacity(colorScheme == .dark ? 0.12 : 0.08), radius: 18, x: 0, y: 10)
    }

    private var journalMoodPicker: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("Mood")
                .font(FontManager.geist(size: 11, weight: .semibold))
                .foregroundColor(colorScheme == .dark ? Color.white.opacity(0.6) : Color.black.opacity(0.56))
                .textCase(.uppercase)
                .tracking(0.4)

            if #available(iOS 17.0, *) {
                journalMoodPickerScrollView
    ...
    private var journalMoodPickerScrollView: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 8) {
                ForEach(JournalMood.allCases) { mood in
                    Button(action: {
                        HapticManager.shared.selection()
                        journalMood = journalMood == mood ? nil : mood
                    }) {
                        HStack(spacing: 6) {
                            Image(systemName: mood.iconName)
                                .font(.system(size: 11, weight: .semibold))
                        ...
                        .foregroundColor(journalMood == mood ? .black : (colorScheme == .dark ? .white : .black))
                        .padding(.horizontal, 10)
                        .padding(.vertical, 8)
                        .background(
                            Capsule()
                        ...
                        .overlay(
                            Capsule()
                                .stroke(
                                    journalMood == mood
                                    ? journalAccentColor.opacity(0.0)
                                    : journalAccentColor.opacity(colorScheme == .dark ? 0.2 : 0.16),
                                    lineWidth: 1
                                )
                        )
                    }
    ...
    private var weeklyRecapMoodSummaryCard: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text("Week moods")
                .font(FontManager.geist(size: 11, weight: .semibold))
                .foregroundColor(colorScheme == .dark ? Color.white.opacity(0.62) : Color.black.opacity(0.54))
                .textCase(.uppercase)
                .tracking(0.4)
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 22)
                .stroke(journalAccentColor.opacity(colorScheme == .dark ? 0.24 : 0.18), lineWidth: 1)
        )
    }

    private func closeReadOnlyRecap() {
        notesManager.isViewingNoteInNavigation = false
        dismiss()
    }
    ...
    private func insertTodoAtCursor() {
        if isContentFocused {
            applyFormattingAction(.checklist)
            return
        }

        // Fallback when editor is not focused yet.
        if content.isEmpty {
            content = "- [ ] "
                                ...
                                Task {
                                    if let extracted = try await AttachmentService.shared.loadExtractedData(for: attachment.id) {
                                        await MainActor.run {
                                            ...
                                            self.extractedDataSheet = extracted
                                        }
                                    }
                                }
                            },
        ...
        VStack(spacing: 8) {
            ...
            HStack(spacing: 10) {
                ...
                Button {
                    ...
                    sleekPrimaryActionIcon(systemName: "textformat.size", isActive: showingFormattingBar)
                }
                ...
                Menu {
                    ...
                    if note != nil && !isWeeklyRecapEditor {
                        Divider()
                        Button(role: .destructive) {
                            HapticManager.shared.delete()
                            deleteNote()
    ...
    private var sleekBottomBarBackground: some View {
        ZStack {
            ...
            RoundedRectangle(cornerRadius: 26)
                .fill(
                    LinearGradient(
                        gradient: Gradient(colors: [
                            isJournalStyledNote
                                ? (colorScheme == .dark ? journalAccentColor.opacity(0.18) : Color.white.opacity(0.72))
                                : (colorScheme == .dark ? Color.white.opacity(0.08) : Color.white.opacity(0.62)),
                            isJournalStyledNote
                                ? (colorScheme == .dark ? Color.white.opacity(0.04) : journalAccentColor.opacity(0.12))
            ...
            RoundedRectangle(cornerRadius: 26)
                .stroke(
                    isJournalStyledNote
                        ? journalAccentColor.opacity(colorScheme == .dark ? 0.26 : 0.16)
    ...
    private func sleekPrimaryActionFillColor(isActive: Bool) -> Color {
        if isActive {
            if isJournalStyledNote {
                return journalAccentColor.opacity(colorScheme == .dark ? 0.26 : 0.18)
            }
            return colorScheme == .dark ? Color.white.opacity(0.2) : Color.black.opacity(0.13)
        }

        if isJournalStyledNote {
            return colorScheme == .dark ? Color.white.opacity(0.08) : journalAccentColor.opacity(0.1)
        }
        return colorScheme == .dark ? Color.white.opacity(0.08) : Color.black.opacity(0.05)
    }

    private var sleekPrimaryActionStrokeColor: Color {
        if isJournalStyledNote {
            return journalAccentColor.opacity(colorScheme == .dark ? 0.2 : 0.12)
        }
    ...
    private func sleekPrimaryActionIcon(systemName: String, isActive: Bool = false) -> some View {
        Image(systemName: systemName)
            ...
            .background(
                Capsule()
                    .fill(sleekPrimaryActionFillColor(isActive: isActive))
            )
            .overlay(
                Capsule()
                    .stroke(sleekPrimaryActionStrokeColor, lineWidth: 0.5)
            )
    }
    ...
    private func openAttachmentPreview() {
        ...
        Task {
            if let data = try? await AttachmentService.shared.downloadFile(from: att.storagePath) {
                ...
                await MainActor.run {
                    filePreviewItem = FilePreviewItem(url: tmp)
                }
            }
        }
    }
                        ...
                        if !imageAttachments.isEmpty {
                            ...
                            Task {
                                if let data = try? await AttachmentService.shared.downloadFile(from: att.storagePath) {
                                    let tmp = FileManager.default.temporaryDirectory.appendingPathComponent(att.fileName)
                                    try? data.write(to: tmp)
                                    await MainActor.run { filePreviewItem = FilePreviewItem(url: tmp) }
                                }
                            }
                        }
                    } label: {
    ...
    private func onAppearAction() {
        ...
        if let note = note {
            ...
            journalWeekStartDate = note.journalWeekStartDate
            journalMood = note.journalMood

            // Parse content and load images - defer to avoid layout glitches
            DispatchQueue.main.async {
                self.attributedContent = self.parseContentWithImages(note.displayContent)
            }
            ...
            if note.attachmentId != nil {
                Task {
                    do {
                        ...
                        if let attachments = AttachmentService.shared.attachments.first {
                            await MainActor.run {
        ...
        if note == nil {
            ...
            content = Note.stripJournalMetadata(from: initialContent)
            noteKind = initialNoteKind
            journalDate = initialJournalDate
            journalWeekStartDate = initialJournalWeekStartDate
            journalMood = Note.extractJournalMood(from: initialContent)
            attributedContent = parseContentWithImages(Note.stripJournalMetadata(from: initialContent))
        }
    ...
    private func refreshJournalHeaderSummary() {
        journalHeaderSummaryTask?.cancel()
        journalHeaderSummaryTask = nil

        guard isJournalEntryEditor else {
            journalHeaderAISummary = nil
            return
        }

        let sourceText = content.trimmingCharacters(in: .whitespacesAndNewlines)
        guard sourceText.count >= 80 else {
            journalHeaderAISummary = nil
            return
        }

        let noteId = currentNoteId ?? editingNote?.id ?? note?.id
        if let noteId,
           let cachedSummary = JournalHeaderSummaryCache.summary(for: noteId, text: sourceText) {
            journalHeaderAISummary = cachedSummary
            return
        }

        journalHeaderSummaryTask = Task {
            do {
                let summary = try await openAIService.summarizeJournalHeaderText(sourceText)
                guard !Task.isCancelled else { return }
                let cleanedSummary = sanitizeJournalHeaderSummary(summary)

                if let noteId, !cleanedSummary.isEmpty {
                    JournalHeaderSummaryCache.store(cleanedSummary, for: noteId, text: sourceText)
                }

                await MainActor.run {
                    if !cleanedSummary.isEmpty {
                        journalHeaderAISummary = cleanedSummary
                    } else {
                        journalHeaderAISummary = nil
                    }
                }
    ...
    private func sanitizeJournalHeaderSummary(_ summary: String) -> String {
        let cleaned = summary
    ...
    private func scheduleInitialTitleFocus() {
        ...
        let firstAttempt = DispatchWorkItem {
            isTitleFocused = true
        }

        let secondAttempt = DispatchWorkItem {
            if !isTitleFocused {
                isTitleFocused = true
            }
        }

        pendingTitleFocusWorkItems = [firstAttempt, secondAttempt]
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.2, execute: firstAttempt)
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.4, execute: secondAttempt)
    }

    private func cancelPendingTitleFocusRequests() {
        pendingTitleFocusWorkItems.forEach { $0.cancel() }
        pendingTitleFocusWorkItems.removeAll()
    }
    ...
    private func moveFocusFromTitleToBody() {
        ...
        // Multiple focus attempts handle UIKit/SwiftUI responder timing races.
        DispatchQueue.main.async {
            isContentFocused = true
        }
    ...
    private func saveNoteAndDismiss() {
        let trimmedTitle = title.trimmingCharacters(in: .whitespacesAndNewlines)
        let contentToSave = persistedEditorContent()
    ...
    private func performSave(title: String, content: String) async -> UUID? {
        if let existingNote = editingNote {
            ...
            updatedNote.kind = noteKind
            updatedNote.journalDate = journalDate
            updatedNote.journalWeekStartDate = journalWeekStartDate
            updatedNote.dateModified = Date()
            ...
            notesManager.updateNote(updatedNote)
            editingNote = updatedNote
            if updatedNote.isJournalEntry {
                await JournalService.shared.ensureWeeklyRecapIfNeeded(referenceDate: updatedNote.journalDate ?? Date())
            }
            ...
            var newNote = Note(
                ...
                journalWeekStartDate: journalWeekStartDate
            )
            ...
            if newNote.isJournalEntry {
                await JournalService.shared.ensureWeeklyRecapIfNeeded(referenceDate: newNote.journalDate ?? Date())
            }
    ...
    private func performAutoSave() async {
        ...
        var updatedNote = existingNote
        updatedNote.title = title
        updatedNote.content = persistedEditorContent()
        updatedNote.kind = noteKind
        updatedNote.journalDate = journalDate
        updatedNote.journalWeekStartDate = journalWeekStartDate
        updatedNote.dateModified = Date()
    ...
    private func persistedEditorContent() -> String {
        if isJournalEntryEditor {
            return Note.applyJournalMood(journalMood, to: content)
        }
        return Note.stripJournalMetadata(from: content)
    }
    ...
    private func convertMarkdownToAttributedString(_ text: String) -> NSAttributedString {
        let textColor = colorScheme == .dark ? UIColor.white : UIColor.black
        return MarkdownParser.shared.parseMarkdown(text, fontSize: 14, textColor: textColor)
    }
        ...
        Task {
            ...
            do {
                ...
                // Priority 3: Create a new note
                    ...
                    let newNote = Note(
                        title: self.title.isEmpty ? "Untitled" : self.title,
                        content: self.content,
                        folderId: self.selectedFolderId,
                        kind: self.noteKind,
                        journalDate: self.journalDate,
                        journalWeekStartDate: self.journalWeekStartDate
                    )
                ...
                do {
                    ...
                    if shouldUpdateNote {
                        ...
                        _ = await self.notesManager.updateNoteAndWaitForSync(noteToUpdate)
...
struct NotesArchiveView: View {
    ...
    @StateObject private var notesManager = NotesManager.shared
    @StateObject private var archiveState = NotesArchiveState()
    @Environment(\.colorScheme) private var colorScheme
    @State private var expandedMonths: Set<String> = []
    @State private var didInitializeExpandedMonths = false

    private var archiveMonthGroups: [NotesArchiveMonthGroup] {
        archiveState.monthGroups
    }

    private var totalVisibleNotes: Int {
        archiveState.totalVisibleNotes
    }

    private var archiveFavoriteNotes: [Note] {
        archiveMonthGroups
            .flatMap(\.notes)
            .filter(\.isPinned)
    ...
    var body: some View {
        ...
        .navigationBarTitleDisplayMode(.inline)
        .onAppear {
            initializeExpandedMonthsIfNeeded()
        }
        .onChange(of: archiveMonthGroups.map(\.month)) { _ in
            initializeExpandedMonthsIfNeeded()
        }
    }
    ...
    private var archiveScrollView: some View {
        ScrollView(.vertical, showsIndicators: false) {
            LazyVStack(alignment: .leading, spacing: 20) {
                if archiveMonthGroups.isEmpty {
                    emptyState
                } else {
                    if !archiveFavoriteNotes.isEmpty {
                        archiveFavoritesSection
                    }

                    archiveSummaryRow

                    ForEach(archiveMonthGroups, id: \.month) { monthGroup in
                        monthSectionCard(monthGroup)
                    }
                }
    ...
    private var archiveFavoritesSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 10) {
                Text("FAVORITES")
                    .font(FontManager.geist(size: 12, weight: .semibold))
                ...
                Text("\(archiveFavoriteNotes.count)")
                    .font(FontManager.geist(size: 11, weight: .semibold))
                    .foregroundColor(Color.appTextPrimary(colorScheme))
            ...
            VStack(spacing: 10) {
                ForEach(archiveFavoriteNotes) { note in
                    archiveNoteRow(note)
                }
            }
            .padding(8)
            .background(monthSectionBackground)
        }
    }

    private var archiveSummaryRow: some View {
        HStack(spacing: 8) {
            Text("\(totalVisibleNotes) \(totalVisibleNotes == 1 ? "note" : "notes")")
                .font(FontManager.geist(size: 13, weight: .semibold))
                .foregroundColor(Color.appTextPrimary(colorScheme))
            ...
            Text("\(archiveMonthGroups.count) \(archiveMonthGroups.count == 1 ? "month" : "months")")
                .font(FontManager.geist(size: 13, weight: .medium))
                .foregroundColor(Color.appTextSecondary(colorScheme))
        }
        .padding(.horizontal, 4)
    ...
    private func monthSectionCard(_ monthGroup: NotesArchiveMonthGroup) -> some View {
        VStack(alignment: .leading, spacing: 10) {
            Button(action: {
                withAnimation(.easeInOut(duration: 0.22)) {
                    if expandedMonths.contains(monthGroup.month) {
                        expandedMonths.remove(monthGroup.month)
                    } else {
                        expandedMonths.insert(monthGroup.month)
                    }
                }
            }) {
                HStack(spacing: 10) {
                    Text(monthGroup.month.uppercased())
                        .font(FontManager.geist(size: 12, weight: .semibold))
                        .foregroundColor(Color.appTextSecondary(colorScheme))
                        .tracking(0.8)

                    Text("\(monthGroup.notes.count)")
                        .font(FontManager.geist(size: 11, weight: .semibold))
                        .foregroundColor(Color.appTextPrimary(colorScheme))
                        .padding(.horizontal, 9)
                        .padding(.vertical, 4)
                    ...
                    Image(systemName: expandedMonths.contains(monthGroup.month) ? "chevron.up" : "chevron.down")
                        .font(.system(size: 11, weight: .semibold))
                        .foregroundColor(Color.appTextSecondary(colorScheme))
                }
                .padding(.horizontal, 4)
    ...
    private func archiveNoteRow(_ note: Note) -> some View {
        HStack(alignment: .center, spacing: 12) {
                ...
                HStack(spacing: 6) {
                    if note.isLocked {
                        Image(systemName: "lock.fill")
                            ...
                            .foregroundColor(archiveMetaColor)
                    }
                    ...
                    if note.reminderDate != nil {
                        Image(systemName: note.isReminderDue ? "bell.badge.fill" : "bell.fill")
                            .font(.system(size: 12, weight: .medium))
                            .foregroundColor(note.isReminderDue ? Color.claudeAccent : archiveMetaColor)
                    }
            ...
            Image(systemName: note.isPinned ? "pin.fill" : "arrow.up.right")
                .font(.system(size: 12, weight: .semibold))
                .foregroundColor(note.isPinned ? Color.appTextPrimary(colorScheme) : archiveMetaColor)
    ...
    private var archiveMetaColor: Color {
        Color.appTextSecondary(colorScheme)
    }
    ...
    private func initializeExpandedMonthsIfNeeded() {
        guard !didInitializeExpandedMonths else { return }
        guard !archiveMonthGroups.isEmpty else { return }

        expandedMonths = Set(archiveMonthGroups.map(\.month))
        didInitializeExpandedMonths = true
    }
}
