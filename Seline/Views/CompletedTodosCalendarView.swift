...
//
//  CompletedTodosCalendarView.swift
//  Seline
//
...
struct CompletedTodosCalendarView: View {
    ...
    private func dayView(_ day: Day) -> some View {
        let todosOnDay = completedTodos.filter { Calendar.current.isDate($0.dueDate, inSameDayAs: day.date) }
    ...
    @ViewBuilder
    private var todosForSelectedDateView: some View {
        if let selectedDate = selectedDate {
            let todos = completedTodos.filter { Calendar.current.isDate($0.dueDate, inSameDayAs: selectedDate) }
            
            VStack(alignment: .leading) {
                Text("Todos on \(selectedDate, formatter: Self.dateFormatter)")
                    .font(.headline)
                    .padding()
                ...
                if todos.isEmpty {
                    ...
                    List(todos) { todo in
                        TodoRow(todo: todo, onToggleComplete: {}, onDelete: {}, onEdit: {})
                    }
                }
            }
    ...
    private func fetchCompletedTodos() {
        self.completedTodos = todoManager.fetchCompletedTodos(for: date)
    }
...
struct CompletedTodosCalendarView_Previews: PreviewProvider {
