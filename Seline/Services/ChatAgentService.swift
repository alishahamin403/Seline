    ...
    private func plannerMessages(
        ...
        var messages: [[String: Any]] = [
            inputMessage(
                ...
                - CRITICAL — When you find a specific event or appointment (dentist, doctor, gym class, meeting, any task/event), immediately enrich by running these in parallel: (1) aggregate_seline scope=[receipt] time_range=that exact date to find spending, (2) search_seline_records for visits on that date to find the physical location, (3) search_seline_records for notes or emails from that date. Never stop at the event record alone.
