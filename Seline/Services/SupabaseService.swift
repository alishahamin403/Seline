...
class SupabaseService: ObservableObject {
    ...
    func upsertUser(selineUser: SelineUser) async throws -> SupabaseUser {
        ...
        // Prepare user data for HTTP request
        let userData: [String: Any] = [
            "email": selineUser.email,
            "name": selineUser.name,
            "profile_image_url": selineUser.profileImageURL as Any,
            "google_id": selineUser.id,
            "access_token_encrypted": selineUser.accessToken as Any,
            "refresh_token_encrypted": selineUser.refreshToken as Any,
    ...
    func updateUserTokens(userID: UUID, accessToken: String, refreshToken: String?, expirationDate: Date?) async throws {
        let _ = isoFormatter.string(from: Date()) // Would be used in real implementation
        let _ = expirationDate.map { isoFormatter.string(from: $0) } // Would be used in real implementation

        ProductionLogger.logAuthEvent("[Supabase] Updating tokens for user: \(userID)")

        // Real implementation would execute:
        // UPDATE users SET access_token = $1, refresh_token = $2, token_expiration_date = $3, updated_at = $4 WHERE id = $5
    }
...
extension Notification.Name {
    static let supabaseEmailUpdated = Notification.Name("supabaseEmailUpdated")
    static let supabaseSyncUpdated = Notification.Name("supabaseSyncUpdated")
}
