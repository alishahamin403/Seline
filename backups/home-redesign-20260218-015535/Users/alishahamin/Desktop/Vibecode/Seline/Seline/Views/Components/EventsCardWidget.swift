...
struct EventsCardWidget: View {
    ...
    private var sortedEvents: [TaskItem] {
        sortTasksForDisplay(selectedDateEvents)
    }
    ...
    private func filterTypeID(_ filterType: TimelineEventColorManager.FilterType) -> String {
        switch filterType {
        case .personal:
            return "personal"
        case .personalSync:
    ...
    private struct EventCategorySection: Identifiable {
        let id: String
        let title: String
        let tasks: [TaskItem]
    }

    private var categorizedEvents: [EventCategorySection] {
        let grouped = Dictionary(grouping: sortedEvents) { filterType(from: $0) }

        return grouped
            .map { filterType, tasks in
                let sortedTasks = sortTasksForDisplay(tasks)
                return EventCategorySection(
                    id: filterTypeID(filterType),
                    title: filterDisplayNameForType(filterType),
                    tasks: sortedTasks
                )
            }
            .sorted { lhs, rhs in
                guard let lhsFirst = lhs.tasks.first, let rhsFirst = rhs.tasks.first else {
                    return lhs.title.localizedCaseInsensitiveCompare(rhs.title) == .orderedAscending
                }
                if taskComesBefore(lhsFirst, rhsFirst) { return true }
                if taskComesBefore(rhsFirst, lhsFirst) { return false }
                return lhs.title.localizedCaseInsensitiveCompare(rhs.title) == .orderedAscending
            }
    }

    private func sortTasksForDisplay(_ tasks: [TaskItem]) -> [TaskItem] {
        tasks.sorted(by: taskComesBefore)
    }

    private func taskComesBefore(_ task1: TaskItem, _ task2: TaskItem) -> Bool {
        let calendar = Calendar.current
        ...
        // Both have scheduled times - compare by time-of-day
    ...
    private func eventRow(_ task: TaskItem) -> some View {
        ...
        let circleColor = colorScheme == .dark ? Color.white : Color.black
        ...
        return HStack(spacing: 10) {
            ...
            Image(systemName: isTaskCompleted ? "checkmark.circle.fill" : "circle")
                ...
                .foregroundColor(isTaskCompleted ? circleColor : circleColor.opacity(0.35))
                .onTapGesture {
    ...
    private func categorySectionHeader(_ section: EventCategorySection) -> some View {
        HStack(spacing: 6) {
            Text(section.title)
                .font(FontManager.geist(size: 11, weight: .semibold))
                .foregroundColor(colorScheme == .dark ? .white.opacity(0.82) : .black.opacity(0.78))

            Text("\(section.tasks.count)")
                .font(FontManager.geist(size: 10, weight: .medium))
                .foregroundColor(colorScheme == .dark ? .white.opacity(0.56) : .black.opacity(0.5))
        }
        ...
        .overlay(
            Capsule()
                .stroke(colorScheme == .dark ? Color.white.opacity(0.16) : Color.black.opacity(0.1), lineWidth: 0.5)
        )
    }
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                if sortedEvents.isEmpty {
                    ...
                    ForEach(categorizedEvents) { section in
